...
// InsightWeeks function to fetch data and calculate average pain level
func PhysicalHealthInsightMonths(c *fiber.Ctx) error {
    var (
        logEntryColl = database.GetCollection("logEntry")
        ctx          = c.Context()
    )
 
    monthStartDate := c.Query("firstDate")
    monthEndDate := c.Query("endDate")
    startDate, err := time.Parse("2006-01-02",  monthStartDate)
    if err != nil {
    ...
    endDate, err := time.Parse("2006-02-02", monthEndDate )
    if err != nil {
        return c.Status(fiber.StatusBadRequest).JSON(logEntry.InsightResDto{
            Status:  false,
    ...
    daysInRange := int(endDate.Sub(startDate)) + 1
    ...
    dayData := make([]logEntry.InsightRes, 7)
    ...
    responseData := make([]logEntry.InsightRes, 0)
    ...
    response := logEntry.InsightResDto{
        Status:  true,
